...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		Detector yolo_detector("yolov3.cfg", "yolov3.weights", device);
...
#endif
		int num_faces_found = findFaces(imagesPath, detector_dnn, yolo_detector, sp, faces, imgToFaces, output_file, min_size, max_size, true, dfxml_doc, workspace);
			...
			const std::string workspace)
{
	...
	for (size_t i = 0; i < path_to_images.size(); i++) {
		...
		for(auto obj : detections) {
			// Index for person
			if (obj.obj_id == 0) {
				cv::Mat objMat(img(cv::Rect(obj.x, obj.y, obj.w, obj.h)));

				dlib::matrix<dlib::rgb_pixel> dlibFrame;
				dlib::cv_image<dlib::bgr_pixel> temp_img(objMat);
				...
				{
					output_file << path_to_images[i].filename().string() + "\n";
					dlib::save_png(dlibFrame, workspace + "\\annotated\\" + path_to_images[i].filename().string());
				}
