...
{
	pugi::xml_node file_obj = this->addFileObject(parent, file_path);
...
void DFXMLCreator::add_recognition_match(
	pugi::xml_node &file_object_node, 
	const char * file_path,
...
{
	pugi::xml_node file_obj = this->addFileObject(file_object_node, file_path);

	pugi::xml_node recognition_node = file_obj.append_child("face_recognition_match");
	pugi::xml_node descriptor_distance = recognition_node.append_child("face_descriptor_distance");
	descriptor_distance.text().set(distance);
	pugi::xml_node match_node = recognition_node.append_child("matches_with");
	match_node.text().set(match_image_name);	
}

pugi::xml_node DFXMLCreator::addFileObject(pugi::xml_node &file_object_node, const char * file_path) {

	boost::filesystem::path p(file_path);
	uintmax_t f_size =  boost::filesystem::file_size(p);
	pugi::xml_node file_obj = file_object_node.append_child("fileobject");
	...
	{
		pugi::xml_node hash_nodeSHA1 = file_obj.append_child("hashdigest");
		hash_nodeSHA1.append_attribute("type") = "sha1";
		...
		hash_nodeSHA1.text().set(tmphash);
	}

	return file_obj;
}

void DFXMLCreator::add_detection(pugi::xml_node &file_object_node, dlib::mmod_rect face) {
	//pugi::xml_node detection_node = file_object_node.append_child("facialdetection");

	std::stringstream ss;
	rectangle rec = face.rect;
	ss << rec.left() << " "
		<< rec.top() << " "
		<< rec.right() << " "
		<< rec.bottom();

	pugi::xml_node face_node = file_object_node.append_child("face");
	face_node.text().set(ss.str().c_str());
	pugi::xml_node score = face_node.append_child("confidence_score");
	score.text().set(face.detection_confidence);
}